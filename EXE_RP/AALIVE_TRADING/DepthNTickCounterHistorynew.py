...
####        print buylinesize, 'buyline'
        ...
        showflag = 'no'
        if abs(totalratio) > 111.20 or totalratio < -0.95 :
            showflag = 'show'
...
##################        print "\n" * 50
        tline = (' TOT BOOK %4d x %4d = %4d | %4.1f  %s  \n' % (totalbuy/divfactor, totalsell/divfactor, totdiff/divfactor, totalratio,time))
        wline = (' WHOLE BOOK %4d x %4d = %4d | %4.1f %s' % (totalbuy/divfactor, totalsell/divfactor, totdiff/divfactor, totalratio, time))
...
##    headnum +=5
    lines = presort(sym,'dom',headnum,tailnum)
    bla = checkDOMticks(sym,10,lines)
    for b in bla:
        print b
    rpu_rp.WriteArrayToTxtfile('allbooks.txt',bla)
    output.append(bla)
########    print bla
    sleep(sleeptime)
    loop+=1
