...
##    RP_Snapshot.snapshot_sym(sym,date,['5mins']) ## need this to create good both bars ## 
    ...
    for r in regionlist:
        print ('%9s %9s %9s %9s %9s %9s %9s %9s %9s %9s' % ('r','tag','LOW','HIGH','CLOSE','LOWTIME','HIGHTIME','startbt','endb','OPEN'))
        for tag in tag_list:
            ctag = r+tag
...
##            print r,tag,ctag,startbartime,endbartime #test
            ...
            print('%9s %9s %9s %9s %9s %9s %9s %9s %9s %9s' % (r,tag,LOW,HIGH,CLOSE,LOWTIME,HIGHTIME,startbartime,endbartime,OPEN))
#####################
