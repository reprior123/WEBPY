...
def write_sqlite(tablename,fieldarray,formatdict,defvaluedict):
    ...
    for line in  ['1']: #fileinput.input('newtablelist'):
        ...
        for fn in fnames:
            fntype = ''
            defvalr=''
            fnformat = formatdict[fn]
            fndefvalue = defvaluedict[fn]
            part1 = '\t' +'Field(' +  '\'' + fn + '\''
            part2 = ''
            charlen = ''
            if 'char('  in fnformat:
                fntype = 'string'
            elif 'int('  in fnformat:
                fntype = 'integer'
            elif  fnformat == 'date':
                fntype = 'date'
            elif 'text'  in fnformat:
                fntype = 'date'
            elif 'decimal'  in fnformat:
                fntype = 'decimal(16,2)'
            elif 'datetime'  in fnformat:
                fntype = 'datetime'
            elif fnformat == 'double':
                fntype = 'double'
            elif fnformat == 'blob' or fnformat == 'longblob':
                fntype = 'blob'
            elif fnformat == 'ENABLE' or fnformat=='(':
                fntype = ''
            else:
                fntype = fnformat
                print fntype
            ...
            if fntype == 'string':            
                charlen = fnformat.split('(')[1].replace(')','')           
                part2 = ', \'' + fntype + '\', length=' + charlen
            elif fntype != '':
                part2 = ', \'' + fntype + '\''
                pass
            else:
            ...
            fncount +=1
##            print fnformat, fndefvalue
            if len(fndefvalue) > 0 and fndefvalue != ' NULL':
                defval = ',default=' + fndefvalue
                pass
            else:
                defval =',default=None'

            reqval = ', required=False)'
            if fncount == len(fnames):
                closerstring = ')'
            else:
                closerstring =','
            if part2 != '' or closerstring == ')':
                print part1 + part2 + defval + reqval + closerstring
                pass
    #############################
...
tablenametarget = 'nxcaddfinindexesmarketdata'
firstinsertline = 'n'
...
for l in rpu_rp.TxtToLines(dbnamefull):
    ...
    if fheaderflag == 'y' and 'CREATE TABLE' not in line  and 'PRIMARY KEY ' not in line and 'INSERT' not in line:
        ...
        fheaderarrayline.append(fheadname)
      
    if 'INSERT INTO' in str(l) and firstinsertline == 'n' :
        fheaderarray.append(fheaderarrayline)
        firstinsertline = 'y'
        rpu_rp.WriteArrayToCsvfileAppend(outputarea +tablename +'dbtable.csv',fheaderarray)
        rpu_rp.WriteArrayToCsvfile(outputarea +tablename +'_fields.csv',fheaderarray)
        
    if 'INSERT INTO' in str(l):
        records = rline.split('|')
        numrecordsnew = len(records)
        if tablename != tablenametarget  and 'securitygroups' not in tablename:
...
##    if 'PRIMARY KEY ' in line:
    if 'UNLOCK TABLES' in line and fheaderflag != 'n':
##        print line
##        print tablename
        if tablename in 'bla':#tnamescontroller:
##            write_sqlite(tablename,fheaderarray,fheadformatdict,fheaddefvaluedict)
            pass
##            print  ' run once'
            fheader = ''
            fheaderarray =[]
            fheaderflag = 'n'
            pass
    elif 'UNLOCK TABLES' in line and fheaderflag == 'n' :
        fheaderflag = 'n'
        pass
    else:
        pass
