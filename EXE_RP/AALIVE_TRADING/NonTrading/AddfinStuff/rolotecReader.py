...
import fileinput
print documents
filein = documents + 'FieldsMatcher.php.txt'

for line in fileinput.input( filein):
    pass
##    print line
    if '=> array(' in str(line):
        rolosym = line.split()[0]
    if '\'apiKey\' =>' in str(line):
        addfinsym = line.split()[2]
        print rolosym,',', addfinsym
...
######    if 'Dumping data for table' in line and 'nxcaddfin' not in tablename:
######        fheaderflag = 'n'
