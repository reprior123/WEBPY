...
import rpu_rp, ibutiles, TicksUtile
############################################
decimalboost = 1
#################
    ...
    for sym in symlist:
        for dur in durlist:
            for ind in indlist:
                ...
                maxval = 2* round(tarray[0],4) #this gives the std rather than max
                mmarray.append(ind)
                mmarray.append(dur)
...
#####################
#################
def makearrayJust2(time,slopebs,signbs,bs,indy2,slope2,sign2,indlabel):  ## assumes one field in postion 0
    arrayout =[]
    c=0
    while c < len(bs):
        newline =[]
        numfields = 8
        f = 0
        flist = [time,slopebs,signbs,bs,indy2,slope2,sign2,indlabel]
        for f in flist:
            if f == indlabel:
...
#################################
def joinArraysStates(tarray,indicator,sign,slope,slopetag,Indtitle,sym,crxxflag,tagarray,highs,lows,bs,crossages):  ## assumes one field in postion 0
    arrayout =[]
    c=0
    while c < len(indicator):
        newline =[]
        f = 0
        flist = [tarray,indicator,sign,slope,slopetag,Indtitle,sym, crxxflag, tagarray, highs, lows, bs,crossages]
        for f in flist:
...
##    print comparetimes
    ...
    bbandma = 20
    if Indtitle == 'pivot':
        indicator = pivotpoint(bshighs,bslows,bs)
        signindicator = indicator
    elif Indtitle == 'R':
        indicator = RS1(bshighs,bslows,bs,'R')
        signindicator = indicator
    elif Indtitle == 'S':
        indicator = RS1(bshighs,bslows,bs,'S')
        signindicator = indicator
    elif Indtitle == 'S2':
        indicator = RS2(bshighs,bslows,bs,'S')
        signindicator = indicator
    elif Indtitle == 'R2':
        indicator = RS2(bshighs,bslows,bs,'R')
        signindicator = indicator
        ...
        indicator = MACDdiverg(bs)
        signindicator = indicator
    elif Indtitle == 'kupper':
        indicator = keltner_channels(bshighs,bslows,bs,2,'upper')
        signindicator = indicator
    elif Indtitle == 'klower':
        indicator = keltner_channels(bshighs,bslows,bs,2,'lower')
        signindicator = indicator
    elif Indtitle == 'stddev':
        indicator = stddev(bshighs,bslows,bs,bbandma)
        signindicator = indicator
    elif Indtitle == 'bbandlower':
        indicator = bbands(bshighs,bslows,bs,bbandma,stdvarbb,'lower')#simpleAvgVal,stdvariable)
        signindicator = indicator
    elif Indtitle == 'bbandupper':
        indicator = bbands(bshighs,bslows,bs,bbandma,stdvarbb,'upper')#simpleAvgVal,stdvariable)
        signindicator = indicator
    elif Indtitle == 'kmid':
        indicator = keltner_channel_mid(bshighs,bslows,bs,2)
        signindicator = indicator
    elif Indtitle == 'ema':
        indicator = EMAmvavgToArray(bs,21)
        signindicator = indicator
    elif Indtitle == 'sma200':
        indicator = mvavgToArray(bs,200)
        signindicator = indicator
    elif Indtitle == 'sma50':
        indicator = mvavgToArray(bs,50)
        signindicator = indicator
    elif Indtitle == 'sma100':
        indicator = mvavgToArray(bs,100)
        signindicator = indicator
...
##        timestamparray = comparetimes
        ...
        indicator = StochD((StochK(bshighs,bslows,bs,stochval)),stochval2)
        signindicator = indicator   
    elif Indtitle == 'StochK':
        indicator = StochK(bshighs,bslows,bs,stochval)
        signindicator = indicator
    elif Indtitle == 'Stoch_CROSS':  
        indicator = Stoch_CROSS(StochK(bshighs,bslows,bs,stochval),StochD(StochK(bshighs,bslows,bs,stochval),stochval2))
        signindicator = indicator
    elif Indtitle == 'ATR':
        indicator = ATR(bshighs,bslows,bs,14)
        signindicator = indicator
    elif Indtitle == 'RVIline':
        ind1 = EMAmvavgToArray(RVIsignal(bsopen,bshighs,bslows,bs),rvival2)
        indicator = RVIline(ind1,rvival)
        signindicator = indicator   
    elif Indtitle == 'RVIsignal':
        indicator = EMAmvavgToArray(RVIsignal(bsopen,bshighs,bslows,bs),rvival2)
        signindicator = indicator
    ...
    if Indtitle == 'diffvES':
        arrayout = joinArraysStates(comparetimes,indicator,sign,slope,slopetag,Indtitle,sym,showcrxx,durarray,bshighs,bslows,bs,crossages)
    else:
        arrayout = joinArraysStates(timestamparray,indicator,sign,slope,slopetag,Indtitle,sym,showcrxx,durarray,bshighs,bslows,bs,crossages)
    alist = timestamparray,indicator,sign,slope,slopetag,Indtitle,sym,showcrxx,durarray,bshighs,bslows,bs,crossages
    prevlena = 0
    return arrayout
###########################  RVI      
