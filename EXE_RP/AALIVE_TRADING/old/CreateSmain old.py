...
##            print l
        ...
        if distance < limit:
            if underover == 'under':
                ...
                sflag = 'BUY'
            print ('>>%4s at %8.2f %s|%s |%4.2f |%8.2f > pass #1 or #2? range=%d' % (sflag,spotp,spotid,sym,distance,curprice,limit))
#####################
...
##    print "\n" * 50
    ...
    if loop == 0 :
        ...
        canswer = raw_input('createnewstates? ')
        if canswer == 'y':
            barlist = barlist_All
            make_bars_no_5sec(date,startmode,symbol_list,barlist) #def make_bars_no_5sec(date,startmode,symbol_list,barlist)
            make_states(date,['ES'],barlist,indlist_All) #make_states('fullist',date,['ES'])
            make_states(date,symbol_list,barlist,indlist_All) #make_states('fullist',date,symbol_list)

        tanswer = raw_input('createtable? ')
        if tanswer == 'y':
            rpInd.CreateIndvalueTable(indlist_part1,['1min','5mins','15mins','1hour','1day'],['ES','FDAX','CL'],libarea +'indlevels.csv')
...
####        startmode = 'bartobar' barlist = barlist_78  make_bars_no_5sec(date,startmode,symbol_list,barlist)
        barlist = barlist_All #barlist_Recent
        indlist = indlist_All #indlist_part1 # indlist_part##        indlist = ['bbandlower']
