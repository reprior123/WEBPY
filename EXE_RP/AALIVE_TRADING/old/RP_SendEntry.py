...
##            for  lastsig in [recentsigs[len(recentsigs)-1]]:
            if testmode == 'online':                    
            ...
            for  lastsig in recentsigs:
                ...
                ##########################################
                if testmode == 'online':
                    pricenow = rounderrp(float(get_latest_tick(sym)),tickvalue)
                else:
                    pricenow =1.0
##                print pricenow - limitprice,'is the difference between sig and entry price'
                ########################
                if tside == 'SELL':
                    entrybuffer = addamt
                else:
