...
import  rpu_rp, rpInd, ibutiles, RP_Snapshot, glob, csv, subprocess, datetime, shutil, time, BarUtiles #TicksUtile
from time import sleep, strftime, localtime
import RulesEngine
...
##time_format %H:%M:%S
#################################
def convertTime(timestring,formatout,direction): #convertTime(timeofbar,'dashspace','timetoepoch')
...
####################################
def merge_bar_files(file1,file2,outfile,cutoffmintime):
    cutofftimeEpoch = last_bar_epoch(file1,'noround')
    # last_bar_epoch(filename,roundformat) 
    f1lines = rpu_rp.CsvToLines(file1)  ## usually dload
    f2lines = rpu_rp.CsvToLines(file2) ## usually recent
    f1len = len(f1lines)
    cleanrlines =[]     
    for rline in f2lines:
        if len(rline) > 2:
##            print rline
            bartime_epoch =  convertTime(rline[1],'dashspace','timetoepoch')
            if bartime_epoch > (cutofftimeEpoch + cutoffmintime) and len(rline) > 3:
##                print rline
                f1lines.append(rline)
##    print 'finished merge', file2
    print 'dloadfile len = ',f1len, 'recfile len = ',len(f2lines),'merged = ',len(f1lines)
    rpu_rp.WriteArrayToCsvfile(outfile,f1lines) ## bothfile outfile created here
##############################################
...
#########
def create_bars_from_bars(bararrayin,today,sym,dur,durinseconds,startmode): #startmodes[initialize,bartobar,addonbartoboth]
##    print 'start creator'
    import time
...
##    print onelinenew
    return onelinenew
###########################################
############def format_RTTickoneline_to_Onetick(onelinein,sym):
############    arrayout = []
############    spaceYtime_format = ' %Y-%m-%d %H:%M:%S'
############    oneline =[]
############    txttoremove = ['reqId',',','\'','time','open','high','low','close','volume','wap','count',' ']
############    parsedline = str(onelinein)
############    for item in txttoremove:
############        parsedlinenew = parsedline.replace(item,'')
############        parsedline = parsedlinenew
############    ilist = [sym,timedate,openpr,highpr,lowpr,closepr,sym]
############    for item in ilist:      
############        oneline.append(item)
## this to be rewritten so it runs initialize onece using ddload cutoff and thereafter the both file
###########################
...
def assemble_dur_bars(today,sym,dur,startmode,basisdur,hamode):
    ...
    basisbars = rpu_rp.CsvToLines(basisfile)
    if hamode == 'hamode':
        create_HAbars_from_bars(basisbars,today,sym,dur,durinseconds,startmode) ## creates recentfile
        newbars = create_HAbars_from_bars(basisbars,today,sym,dur,durinseconds,startmode) ## creates recentfile
        bartypetag = '.HA'
    else:
        newbars = create_bars_from_bars(basisbars,today,sym,dur,durinseconds,startmode) ## creates recentfile
        bartypetag = ''
##        for b in newbars:
##            print b
    file1 = DataDown +today+'.'+sym+'.'+dur.replace(' ','')+'.ddload'+bartypetag +'.csv'
    filetempdl = DataDown +today+'.'+sym+'.'+dur.replace(' ','')+'.tempdurs'+bartypetag +'.csv'
    rpu_rp.WriteArrayToCsvfile(filetempdl,newbars)
    file2 = DataDown +today+'.'+sym+'.'+dur.replace(' ','')+'.recent'+bartypetag +'.csv'
    outfile = DataDown +today+'.'+sym+'.'+dur.replace(' ','')+'.both'+bartypetag +'.csv'   
