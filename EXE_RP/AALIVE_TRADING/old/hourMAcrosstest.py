...
symlist=['ES', 'JPY']
for sym in symlist:
    filelist = glob.glob(states + '*' + sym +'*mcross*')
    ##print filelist
    prevtimeepoch = prevdiff =0
    os.system('rm tempf')
    for f in filelist:
        print f
        os.system('cat ' +f + ' >> tempf')
        ##os.system('grep crx tempf | grep 1 > newtempf ')
    os.system('grep 1 tempf | tail -15 > newtempf ')
##        os.system('grep pos  tempf | grep slopedn | tail -15 > newtempf ')
    array = rpu_rp.CsvToLines('newtempf')
...
##
##            diff = timeepoch - prevtimeepoch
##            if diff != prevdiff:
##                print diff, timeraw,prevtimeepoch
##            prevtimeepoch = timeepoch
##            prevdiff = diff
###########
...
while loop < loopmax:
    ...
    print 'cycle heartbeat...serching for sigs in last ',recentlimit
...
##    print command,'com setting timedrift..pricedrift',now
    ...
    for sym in symbol_list:      
        ...
        RTBars5Sec = TicksUtile.create_bars_from_bars(RTTickBarsReformatted,today,sym,'5secs',1) ##RTticks > RTbars[5secRecentBars]
##        print '5sec bars done'
        filetomerge = DataDown +today+'.'+sym+'.'+'5secs'+'.ddload.csv' #. .. only really need to do this once..test for recent file?
...
##        print 'finish tick rewrite'
        ...
        for barsize in barlist :#[barlist[0]]:
            ...
            TicksUtile.merge_bar_files(filetomerge)  #this creates the both file   
            lines = rpu_rp.CsvToLines( DataDown+ today + '.'+sym+'.' + dur.replace(' ','') + '.both.csv')          
####################
            print sym, dur
            for line in lines:
                print line
            macross = rpInd.process_ticks(lines,decimalboost,barsize)
            macd = rpInd.process_MACDticks(lines,decimalboost,barsize)          
