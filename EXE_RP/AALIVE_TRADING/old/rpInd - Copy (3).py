...
def pivotpoint(a1,a2,a3): ## hi,lo,close arrays
    ...
    return arrayout
######################
def RS(a1,a2,a3,mode,levelmode):  ## S1 is the same but with lows ## pivot,high,lo,close arrays
    c=0
    arrayRout =[]
    arraySout =[]
...
###########################
def OBV(closes,volume,maVal):   ## on balance volume slope change precedes mkt flip
    previouscloses = [1]
    for d in closes:
        previouscloses.append(d)
    c=0
    obvs =[]
    while c < len(closes):
        barchange = closes[c] - previouscloses[c]
        if barchange < 0:
            sign = -1
            pass
        elif barchange > 0:
            ...
            sign = 0
        obvlineval = volume[c] * sign
        obvs.append(obvlineval)
##        runtotal on obvs
        c+=1
    runtotalarray =  RunningTotToArray(obvs)
    return runtotalarray
 ##################################3       
## 20period sum of mfvolumes / volume   same as mvaverage?
##    AvgGain = mvavgToArray(Gainarray,14)
##        arrayout.append(round(RSI,2))
...
def create_states_files(sym,dur,date,threshold,newindlist):
    ...
    DurBothyesterday = rpu_rp.CsvToLines( DataDown+ date + '.'+sym+'.' + dur.replace(' ','') + '.both.csv')
...
##    print 'getting states for...',Indtitle,dur,sym,date
    if len(arrayin)  > 3 :
        pass
    else:
...
##    print comparetimes
    ...
    if Indtitle == 'pivot':
        ...
        indicator = RS(bshighs,bslows,bsclose,'R','1')
        signindicator = indicator
    elif Indtitle == 'S1':
        indicator = RS(bshighs,bslows,bsclose,'S','1')
        signindicator = indicator
    elif Indtitle == 'S2':
        indicator = RS(bshighs,bslows,bsclose,'S','2')
        signindicator = indicator
    elif Indtitle == 'R2':
        indicator = RS(bshighs,bslows,bsclose,'R','2')
        signindicator = indicator
