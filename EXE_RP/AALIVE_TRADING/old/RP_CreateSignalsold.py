    ...
    for sym in symbol_list:      
        ...
        for barsize in barlist :
            ...
            for indicator in indlist:  
                ...
                statefile = sigarea +statename + indicator  + '.state.csv'
...
##        macrossstate = statefile = sigarea +statename +'.MACROSS.state.csv'
        ...
        for sig in sorted(recentsigs):
            ...
            if sigcount == len(recentsigs):
                ...
                for durstate in durstatelist:
                    stateinfo += durstate + '\n'
                    for indicator in indlist:  
                        sfile = rpu_rp.CsvToLines(sigarea +sym+'.' + durstate +'.'+ indicator +'.state.csv')
    ...
    ##############################            
                ...
                timedrift = barToNow
                print '==============='
                print timedrift,sym, tside,dur, priceinsignal,barToPrev,sigtime,pricedrift
                print stateinfo
                ######################
                ttype = 'LIM'
                limitprice = bid
