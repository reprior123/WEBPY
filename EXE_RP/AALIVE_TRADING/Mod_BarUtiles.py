...
def show_hi_lo_bar_range(sym,dur,startbartime,endbartime,date):
    ...
    newloprice =99999
    newhiprice = 0.0
    lowtime = hitime = closetime = opentime ='na'
    clsprice = openprice =0.0
    newbars = show_bar_range(sym,dur,startbartime,endbartime,date)
##    print sym,dur,startbartime,endbartime
    for bar in newbars:
##        print bar
        if len(bar) > 2:
            print 'empty?',bar
            ...
            closetime = bar[1]
            openprice = float(bar[2])
            opentime = bar[1]
            lowtime = hitime ='na'
            if lowprice < newloprice:
                newloprice = lowprice
                lowtime = opentime
            if hiprice > newhiprice:
                newhiprice = hiprice
                hitime = opentime
    hilow.append(newloprice)
    hilow.append(newhiprice)
    hilow.append(lowtime)
    hilow.append(hitime)
    hilow.append(clsprice)
    hilow.append(closetime)
    hilow.append(openprice)
    hilow.append(opentime)
    if len(newbars) == 0:
        hilow = ['na','na','na','na','na','na','na','na']
    return hilow
####################
