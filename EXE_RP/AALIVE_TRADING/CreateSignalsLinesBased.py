...
##            print curprice-spotp,spotp,curprice,sym,'spot prices',limit
            if underover == 'under':
                print ('SELL at %8.2f | %s | %4.2f | %8.2f  ...is this pass #1 or #2?' % (spotp,sym,distance,curprice))
...
##                print 'ready TO BUY at ',spotp, 'how manypasses?',curprice,sym,distance
                print ('BUY  at %8.2f | %s | %4.2f | %8.2f  ...is this pass #1 or #2?' % (spotp,sym,distance,curprice))
...
def create_previous_state_array(btestmode,sym,barnum):
    ...
    currentstates =[]
    last1minbar = rpInd.ShowABarofInd(sym,'15mins','mcross',barnum) ## was 1min
...
##    print indsMIDdict
    ...
    sloperule = indsVALNORMALdict[rinddur]
...
###############       
    ...
    if float(float(loop)/factor) == round((loop/factor),1):
        print 'cycle ', loop
        sym = 'ES'
        show_spots(sym,date,limitlines)
########        symd = 'FDAX'
########        limitlinesd = 10.0
...
##                print lbarnum,sym
                lbarnum +=1
                currentstates = create_previous_state_array(btmode,sym,lbarnum)[0]
                timeloop = create_previous_state_array(btmode,sym,lbarnum)[1]
                print timeloop
