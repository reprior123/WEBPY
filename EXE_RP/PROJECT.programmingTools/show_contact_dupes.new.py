...
print justfilename
lines = csvToLines (justfilename,'t')
##print lines
lines2 = lines
...
##    if dupeflag == 'true':
##        print first,last
    if email != '' and first != '' and last != '' and dupeflag != 'true':
##        emailwlineend += email + first + last + '\n'      
        emailwlineend += email + last + '\n'
##        comboWcid += email + first + last + ',' + contactID + ',\n'
        comboWcid += email +  last + ',' + contactID + ',\n'
fileout = open('bla.txt','w')
fileout.write(emailwlineend)
fileout.close()
fileout2 = open('bla2.txt','w')
fileout2.write(comboWcid)
fileout2.close()

os.system('cat bla.txt | sort | uniq  > blauniq.txt')

uniqfile = open('blauniq.txt', 'r')
ulines = uniqfile.readlines()

fullfile = open('bla2.txt', 'r')
alllines = fullfile.readlines()
alldupes = ''
fullcount = 0
for uline in ulines:
    count = 0
    pattern = str(uline.strip())
##    print '|',pattern,'|'
    for aline in alllines:
        if pattern in str(aline):
            count += 1
            if count > 1:
                alldupes += aline.strip() + ',' + str(count) + ',\n'
                fullcount += 1
print fullcount,'dupes'
dupesfile = open('dupes.csv','w')
dupesfile.write(alldupes)
dupesfile.close()    
uniqfile.close()
fullfile.close()
print 'done'
