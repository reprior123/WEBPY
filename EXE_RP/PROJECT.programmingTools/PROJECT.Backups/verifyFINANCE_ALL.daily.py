...
## this bit grabs yesterday based on the most recent file in the directory includes timestamp ##
dirlisting = sorted((os.listdir(backuparea)))
for fname in dirlisting:
    if '.fnames.txt' in fname:
        yesterdayf = fname.split('.')[1]
##        todayf = fname.split('.')[1]
############
todayfiles = backuparea + maintag + '.' + todayf  +'.fnames.txt'
...
##################### 
##os.system('ls -R "' + livedir + '" > fdirs.txt')    ### comment this out to compare current files
filesarray = []
dirsarray = []
dirname = ''
fullname = ''
...
reportfile = backuparea + 'reportfile.' + todayf + timef + '.txt'
os.system('diff ' + oldsnapshot + ' ' + newsnapshot + ' > ' + reportfile)
print 'diff ' + oldsnapshot + ' ' + newsnapshot + ' > ' + reportfile ')
#####
...
emaillist = ['bb;rob.prior@actant.com;' + att]   ## for testing
tab = '\t'
