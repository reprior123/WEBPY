...
$(document).ready(function(){
    var error_msg = $("p.error, span.error").html();
	$("p.error, span.error").remove();
	if(error_msg != null) {
        $("section.main #content .moduleTitle").before('<p class="error-notice"><span class="icon icon-not-verified"></span><span class="err-msg">'+error_msg+'</span></p>');
	}
});
...
SUGAR.ajaxStatusClass.prototype.positionStatus = function() {
	...
	this.statusDiv.style.left = (parseInt($(this.statusDiv).closest('section').css('padding-left')) + $(this.statusDiv).parent().width() / 2 - statusDivWidth / 2) + 'px';
}
